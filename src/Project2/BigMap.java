...
public class BigMap extends JPanel {// implements java.io.Serializable {
	...
	// private ImageIcon IconRead =new ImageIcon ("icons/read.jpg");
	...
	// JButton btRead = new JButton(IconRead);
	// Create buttons
	...
	JLabel name21 = new JLabel("Little B");
	...
	public BigMap() {
		...
		// btRead.setBounds(128, 160, 65, 74);
		// btRead.setToolTipText("点击继续游戏");
	...
	// 玩家a是使用道具的人，玩家b 是道具被作用的对象
	// public static void useTurnProp(Player p1, Player p2) {
	// if (conPlay % 2 == 0)
	// useTurnPropb = true;
	// else
	// useTurnPropa = true;
			...
			if (conPlay % 2 == 0) {
				...
				if (a.getDirection() == 1) {
					if (con2 >= diceResult) {
						timer2.stop();
						a.setLocation(-diceResult);
						FireEvent sign = new FireEvent(a.getLocation(), a, b);
				...
				if (b.getDirection() == 0) {
					if (con2 >= diceResult) {
						...
						FireEvent sign = new FireEvent(
								(b.getLocation() + 19) % 38, b, a);
	...
	// String f = fi.getAbsolutePath()+"\\t.tmp";
	...
	// out.writeObject(shijian);
	// // out.writeInt(panel.getWeek());
	// // out.writeObject(run1);
	// // out.writeObject(BigMap.musicOn);
	//
	// //for (int a = 0;a < panel.getPlayers().length;a++)
	//
	// //for (int a = 0;a < panel.getLand().length;a++)
	// // out.writeObject(panel.getLand()[a]);
	// // 存储玩家
	// out.writeObject(a);
	// out.writeObject(b);
	// //music
	...
	// //land
	// for (int i =0;i<38;i++) {
	// out.writeObject(FireEvent.land[i]);
	// }
	// out.close();
	// }
	// catch(Exception e){
	...
	// String nameOfFile = f.getAbsolutePath();
	// if(f != null){
	//
	// } ;
	// try{
	...
	// // panel.setClickNumber(input.readInt());
	// bigmap.setPlayTimes(input.readInt());
	// panel.setDate(input.readInt());
	// panel.setWeek(input.readInt());
	// run1 = (Run) input.readObject();
	// music = (JavaClip) input.readObject();
	// useProp = (UseProp) input.readObject();
	// for (int a = 0;a < panel.getPlayers().length;a++)
	// panel.getPlayers()[a] = (Player) input.readObject();
	// for (int a = 0;a < panel.getLand().length;a++)
	// panel.getLand()[a] = (Land) input.readObject();
	// panel = (Draw1) input.readObject();
	// input.close();
	// panel.repaint();
	// }
	// catch(Exception e1){
