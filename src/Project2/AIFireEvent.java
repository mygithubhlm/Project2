...
//		land[19]空地
	...
	public AIFireEvent(int location, Player p,Player np,boolean AIplay) {
		...
		case 37: {
			  //电脑玩家
			if (AIplay==true) {
					if (land[location].getOwner().equals("无")) {
						//现金大于1000
					if (p.getCash()>=1000) {
			...
			if (land[location].getOwner()==p.getName()) {
				...
				if (option == 0) {
					...
					if (p.getCash()>500) {
						...
						JOptionPane.showMessageDialog(null, "您的房产已经达到最高级别！");
						}else {
					      JOptionPane.showMessageDialog(null, "恭喜，花费500元升级成功！");
						...
						JOptionPane.showMessageDialog(null, "对不起，你的现金不足，无法升级该土地");
					}
				}
...
//			}
				...
				//现金不够用时要用存款来支付过路费
					...
					int lastMoney = roadMoney - p.getCash();
					if (p.getDeposit()>=lastMoney) {
						JOptionPane.showMessageDialog(null , "啊哦，你踩在了别人的地盘上，并且你的现金"
								+ "\n不够用，所以从你的存款中提取一部分"
								+ "\n被强行交了"+roadMoney+"的过路费~~555");
					    p.setCash(-p.getCash());
					    p.setDeposit(-lastMoney);
						...
						if (haveHouse==false) {
							JOptionPane.showMessageDialog(null, "对不起，您的家产都被卖光交钱了，您以破产"
									+ "\n恭喜玩家"+np.getName()+"获胜！！！");
							System.exit(0);
						}
					}
		...
		case 31:
			...
			if (random==1) {
				...
				if(p.getHouseValue()>np.getHouseValue()) {
					p.setCash(1000);
					JOptionPane.showMessageDialog(null, "公开奖励房产最多的玩家"+p.getName()+"1000元");
				}//如果俩人房产相同时
				else if (p.getHouseValue()==np.getHouseValue()) {
					int ii = (int)(Math.random()*2+1) ;
					if (ii==1){
						p.setCash(1000);
						JOptionPane.showMessageDialog(null, "公开奖励幸运的玩家"+p.getName()+"1000元");	
					}else{
						np.setCash(1000);
						JOptionPane.showMessageDialog(null, "公开奖励幸运的玩家"+np.getName()+"1000元");	
					}
				}else {
					np.setCash(1000);
					JOptionPane.showMessageDialog(null, "公开奖励房产最多的玩家"+np.getName()+"1000元");
				}
			}
			if(random ==2){
				 //event1 : 公开补助房产最少的玩家
				if(p.getHouseValue()>np.getHouseValue()) {
					np.setCash(1000);
					JOptionPane.showMessageDialog(null, "公开补助房产最少的玩家"+np.getName()+"1000元");
				}//如果俩人房产相同时
				else if (p.getHouseValue()==np.getHouseValue()) {
					int ii = (int)(Math.random()*2+1) ;
					if (ii==1){
					...
					p.setCash(1000);
					JOptionPane.showMessageDialog(null, "公开补助房产最少的玩家"+p.getName()+"1000元");
				}
			}
			if (random == 3) {
				//event3 :
			...
			if (random == 4) {
				...
				JOptionPane.showMessageDialog(null, "你们可真幸运，今天你们每人获得一把道具！Cheers！");
			}
		default:
		}
	}
